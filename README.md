...
| 一般設定 | 可選擇輸出到 Excel 同目錄、固定預設目錄，或每次詢問儲存位置 |
...
目前 CI 只建置並發佈 WinForms 桌面版 artifact，不再部署到 Azure App Service。push 到 `main` 會產生 `OrderHelper-win-x64` artifact，供下載交付使用。
...
- **修正**：同廠商同天多批次判斷改為掃描輸出目錄中「今天已產生、檔名含該廠商」的 PDF 數量，不再依賴來源 Excel 檔名或廠商序號是否相同；避免不同檔名的批次無法正確判斷重複而漏加 `-1`/`-2`

### v1.0.19（2026-07-28）

- **新增**：分廠商輸出時，若同一廠商在同一天已產生過 PDF（例如分批處理訂單），不再覆蓋舊檔，改為檔名加上 `-1`、`-2`… 遞增編號；「請開立下個月發票」批次不受此規則影響
- **改善**：PDF 每頁最多筆數預設值由 10 調整為 12

### v1.0.14（2026-06-29）

- **改善**：PDF 備註區「請開立下個月發票」改為更大的紅色粗體字，提升列印辨識度
...
- **修正**：PDF 上方廠商列補回原始格式的格線，讓廠商名稱、mail、訂貨日期與日期值回到有框線分隔的版面

### v1.0.11（2026-05-26）

- **改善**：Excel 匯入只硬性要求訂購單號、品名規格、訂購量、廠商名稱；電話、傳真、料號等欄位缺少時以空白匯入，再交由檢核規則提示
- **改善**：PDF 輸出位置改為三選一：Excel 同目錄、固定預設目錄、每次詢問，避免「自動儲存」與「預設目錄」語意混淆
- **穩定性**：產生 PDF 時使用畫面設定快照並暫停相關控制項，避免背景產生期間設定被改動

### v1.0.10（2026-05-26）

- **新增**：檢核設定改為下拉選單式常用規則，不需輸入正則；支援必填、數字、正整數、大於 0、電話/傳真格式、統編 8 碼、長度上限等
- **改善**：主視窗套用較現代的配色、表格樣式、按鈕樣式與設定頁間距

### v1.0.9（2026-05-25）

- **修正**：各頁按鈕與 checkbox 高度不足導致中文字下半部被截斷，統一調高至 26px（按鈕）/ 24px（checkbox）

### v1.0.8（2026-05-25）

- **修正**：一般設定分頁「瀏覽」按鈕及主視窗「匯出範例」、「分廠商輸出」控制項文字遭截斷

### v1.0.7（2026-05-25）

- **新增**：PDF 樣式設定 → 一般設定 新增「每頁最多筆數」欄位（數字輸入框，預設 10，可調 1–99）

### v1.0.6（2026-05-25）

- **修正**：每頁上限改為 10 筆（同時保留高度安全檢查，品名超長時仍可能少於 10 筆）

### v1.0.5（2026-05-25）

- **修正**：品項序號跨頁重置為 1，改為延續累計（第 1 頁 1–10，第 2 頁 11–20，依此類推）

### v1.0.4（2026-05-25）

**Bug fixes & 新功能**

- **修正**：每頁表格內資料第一列被空白列擠掉（body rect Y 偏移 19.52pt 導致）
- **修正**：備註欄超長文字溢出頁面右邊界，改為自動縮小字型（最小 6pt）
- **修正**：廠商列 vendor row 雙 checkbox 格式還原為原版（□ mail 訂貨日期 □ 日期）
- **新增**：「分廠商輸出」checkbox — 勾選後每家廠商產生獨立 PDF，檔名為 `<原檔名>_<廠商名>_訂購單.pdf`
...
- 多廠商分頁、自動換頁、長品名多行排版
- 資料驗證：必填、數字、正整數、電話/傳真格式、統編 8 碼、長度上限等常用規則，GUI 可編輯
- 醫院資料全欄可設定（發票抬頭、交貨地址、備註等）
- 一般設定：PDF 輸出位置模式、DefaultPdfDirectory、MaxRowsPerPage
